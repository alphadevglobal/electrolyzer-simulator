...
\end{center}
{\textit{\fontsize{11pt}{11pt}\selectfont Karen Moura Fernandes\textsuperscript{1}, Mateus Gomes Macário\textsuperscript{2}, Autor 3\textsuperscript{3}}}
...
Palavras-chave: inserir até cinco palavras-chave, separadas por ponto e vírgula.
}
...
\textbf{Resumo}
\end{tcolorbox}
A crescente demanda por fontes de energia sustentáveis impulsionou o desenvolvimento do hidrogênio verde, cuja produção ocorre via eletrólise da água, utilizando energia renovável e resultando em emissões zero carbono. Este artigo aborda a simulação computacional de eletrolisadores, equipamento que possibilita a eletrólise, explorando a modelagem matemática e os paramêtros que influenciam a perfomance desses dispositivos.
...
\textbf{Metodologia}
\end{tcolorbox}
Foram realizados extensos estudos bibliográficos para definir os modelos matemáticos, e as técnicas computacionais, que melhor auxiliariam na simulação de eletrolisadores. A base teórica do simulador fundamenta-se nas equações clássicas da eletroquímica, adaptadas para os tipos de eletrolisadores que são relevantes a esse estudo. A tensão total do eletrolisador é descrita pela equação de Nernst modificada (\ref{Nernst}), considerando as sobretensões características do processo, sendo $V_{rev}$ a tensão reversível teórica, $\eta_{act}$ a sobretensão de ativação, $\eta_{ohm}$ a sobretensão ôhmica, e $\eta_{conc}$ a sobretensão de concentração; a tensão reversível (\ref{tensãoRev}) é calculada considerando a dependência da temperatura através da equação de Nernst.
...
A sobretensão de ativação, que representa a energia necessária para iniciar as reações eletroquímicas nos eletrodos, é modelada pela equação de Tafel (\ref{Tafel}), onde $\alpha$ é o coeficiente de transferência de carga, $i$ a densidade de corrente operacional, e $i_0$ a densidade de corrente de troca; a sobretensão ôhmica, relacionada à resistência elétrica dos componentes do eletrolisador, é expressa pela lei de Ohm (\ref{Ohm}). 
...
A produção de hidrogênio é calculada através da lei de Faraday (\ref{faraday}), sendo $\eta_F$ a eficiência faradaica, $I$ a corrente total, e $F$ a constante de Faraday, com a eficiência energética do sistema é definida pela equação (\ref{eficiencia}).
...
\begin{equation}
\eta_{energetica} = \frac{V_{rev}}{V_{cell}} \times \eta_F
\label{eficiencia}
\end{equation}

Essa modelagem matemática foi implementada em uma aplicação web interativa, desenvolvida utilizando tecnologias modernas de desenvolvimento frontend. A arquitetura da aplicação baseia-se em React.js, para a interface de usuário, com bibliotecas especializadas para cálculos científicos e visualização de dados. A aplicação foi estruturada em módulos funcionais distintos: Módulo de Simulação Estática para análise de condições operacionais fixas, Módulo de Simulação Dinâmica para análises temporais, Módulo de Análise Térmica focado no efeito da temperatura, e Módulo de Análise Climática Regional incorporando dados climáticos de diferentes regiões globais. Além disso, foi desenvolvida uma metodologia sistemática de análise de sensibilidade, visando identificar os parâmetros mais influentes no desempenho dos eletrolisadores, com os dados sendo expostos na tabela \ref{parametros_eletrolise} do apêndice.\\
...
\textbf{Apêndice}
\end{tcolorbox}
Os dados utilizados para a modelagem dos eletrolisadores seguem nas tabelas abaixo. 

\begin{table}[h!]
\centering
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Parâmetro}        & \textbf{Alcalino}       & \textbf{PEM}            & \textbf{SOEC} \\ \hline
Temperatura operacional ($^\circ$C)   & 25--80         & 25--80       & até 1000 \\ \hline
Densidade de corrente (A/cm$^2$)    & 0.1--2.0      & 0.1--2.0     & -- \\ \hline
Pressão operacional (bar)    & 1--30          & 1--30              & -- \\ \hline
Concentração do eletrólito (\% KOH) & 10--50            & --                      & -- \\ \hline
Área ativa dos eletrodos (cm$^2$) & 1--10000         & 1--10000        & 1--10000\\ \hline
\end{tabular}
\caption{Parâmetros operacionais típicos de diferentes tecnologias de eletrólise da água.}
\label{parametros_eletrolise}
\end{table}
